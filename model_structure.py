...
from attention import scMoAnnoPretrain,CrossModalAttention
...
class MCF_DDI(nn.Module):
    def __init__(self, net_params):
        ...
        # self.lin = nn.Sequential(
        #
        #     KAN([hidden_dim * 6, 64, hidden_dim * 4]),
        #     # nn.PReLU(),
        ...
        # )
        self.lin = nn.Sequential(
            KAN([hidden_dim * 6, 128, hidden_dim * 3]),

            KAN([hidden_dim * 3, 256, hidden_dim])
        )
        self.graph_pred_linear = nn.Identity()
        self.rmodule = nn.Embedding(86, hidden_dim)#在drugbank中有86中反应类型
        #self.rmodule = nn.Embedding(2, hidden_dim)
        ...
        self.lin_sim = nn.Linear(1705, hidden_dim)#处理相似矩阵
        #self.lin_sim = nn.Linear(544, hidden_dim)
        self.FingerprintNet = FingerprintNet_()
        self.scTransSort = scTransSort()
        self.CrossModalAttention = CrossModalAttention()
        self.scMoAnnoPretrain = scMoAnnoPretrain()
    ...
    def Fusion(self, sub, data, finger, sim_ht):
        ...
        dg_n = nn.BatchNorm1d(d_g.shape[1]).to(self.device)
        dg = dg_n(d_g)

        # sim_ht = self.lin_sim(sim_ht.float())#线性层（1024，1706）--》（1024，128）
        sim_n = nn.BatchNorm1d(sim_ht.shape[1]).to(self.device)
        simht = sim_n(sim_ht)

        finger_n1 = nn.BatchNorm1d(finger.shape[1]).to(self.device)
        fingern = finger_n1(finger)

        global_future =  torch.concat([dg,simht,fingern],dim=-1)
        ...
        # # sim_ht = self.lin_sim(sim_ht.float())#线性层（1024，1706）--》（1024，128）
        # #使用交叉注意力
        # d_g1,finger_c1 = self.scMoAnnoPretrain(d_g,finger_c)
        # d_g2, sim_ht1 = self.scMoAnnoPretrain(d_g, sim_ht)
        # sim_ht2, finger_c2 = self.scMoAnnoPretrain(sim_ht, finger_c)
        #==========================================================================
        # norm_d_g1 = nn.BatchNorm1d(d_g1.shape[1]).to(self.device)
        # d_g1 = norm_d_g1(d_g1)
        # norm_d_g2 = nn.BatchNorm1d(d_g2.shape[1]).to(self.device)
        # d_g2 = norm_d_g2(d_g2)
        #
        # norm_finger_c1 = nn.BatchNorm1d(finger_c1.shape[1]).to(self.device)
        # finger_c1 = norm_finger_c1(finger_c1)
        # norm_finger_c2 = nn.BatchNorm1d(finger_c2.shape[1]).to(self.device)
        # finger_c2= norm_finger_c2(finger_c2)
        #
        # norm_sim_ht1 = nn.BatchNorm1d(sim_ht1.shape[1]).to(self.device)
        # sim_ht1 = norm_sim_ht1(sim_ht1)
        # norm_sim_ht2 = nn.BatchNorm1d(sim_ht2.shape[1]).to(self.device)
        # sim_ht2 = norm_sim_ht2(sim_ht2)

        # dg = d_g1+d_g2
        # fg = finger_c1+finger_c2
        # si = sim_ht1+sim_ht2
        # global_feature = dg+fg+si
        # #global_feature = torch.concat([dg,fg,si],dim=-1)
        ...
        # d_f = norm_df(d_f)
        # d_g2, sim_ht1 = self.scMoAnnoPretrain(d_f, sim_ht)
        # global_feature = d_g2 + sim_ht1
        # norm_global = nn.BatchNorm1d(global_feature.shape[1]).to(self.device)
        # global_feature = norm_global(global_feature)
        # return global_feature
        #====================================
        # d_g1, f_2 = self.scMoAnnoPretrain(d_g, finger_c)
        # d_g2, s_1 = self.scMoAnnoPretrain(d_g, sim_ht)
        # f_1, s_2 = self.scMoAnnoPretrain(finger_c, sim_ht)
        # dg = d_g1+d_g2
        # f = f_1+f_2
        # s = s_1+s_2
        ...
        # Max = global_max_pool(sub, data.batch)
        # Mean = global_mean_pool(sub, data.batch)
        # sub_ = torch.cat([Max,Mean],dim=-1)
        # sub_ = sub_.float()
        # rfeat = self.rmodule(rel)
        # sub_ = (self.lin(sub_) * rfeat).sum(-1)
        #
        # finger_ = torch.cat([finger, finger], dim=-1)
        # finger_ = finger_.float()
        # finger_ = (self.lin(finger_) * rfeat).sum(-1)
        #
        # sim_ht_ = sim_ht.float()
        # sim_ht_ = (self.lin(sim_ht_) * rfeat).sum(-1)
        #
        # global_feature = 0.3*sub_+0.3*finger_+0.3*sim_ht_
        # return global_feature
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger,tail_finger,rel, sim1, sim2,sim_dt_h,sim_dt_t):
        ...
        pair = d_g1+d_g2
        pair_n = nn.BatchNorm1d(pair.shape[1]).to(self.device)
        pair = pair_n(pair)
        pair = pair.float()
        rfeat = self.rmodule(rel)
        logit = (self.lin(pair) * rfeat).sum(-1)
...
class GNN_Sim(nn.Module):
    ...
    def Fusion(self, sub, data, sim_ht):
        ...
        norm_dg = nn.BatchNorm1d(d_g.shape[1]).to(self.device)
        d_g = norm_dg(d_g)
        # global_feature = torch.cat([d_g,finger,sim_ht],dim=-1)
        # return global_feature

        #================================================
        # sim = self.lin_sim(sim.float())#线性层（1024，1706）--》（1024，128）
        # 使用交叉注意力
        d_g1, sim_ht1 = self.scMoAnnoPretrain(d_g, sim_ht)
        global_feature = d_g1 + sim_ht1
        norm_global = nn.BatchNorm1d(global_feature.shape[1]).to(self.device)
        global_feature = norm_global(global_feature)
        return global_feature
    ...
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger,tail_finger,rel, sim1, sim2,sim_dt_h,sim_dt_t):
        ...
        for i,conv in enumerate(self.layers):#3层注意力
            ...
            h2,e2 = conv(g2, h2, e2)
        #=======================================
        d_g1 = self.Fusion(h1,h_data,sim_12)
        d_g2 = self.Fusion(h2,t_data,sim_dt_ht)
...
class Sim_(nn.Module):
    ...
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger,tail_finger,rel, sim1, sim2,sim_dt_h,sim_dt_t):
        ...
        pair = torch.cat([sim_12, sim_dt_ht], dim=-1)
        pair = pair.float()
        rfeat = self.rmodule(rel)
        logit = (self.lin(pair) * rfeat).sum(-1)
        return logit
class GNN_(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        num_atom_type = net_params['num_atom_type']  # 70
        num_bond_type = net_params['num_bond_type']  # 6
    ...
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger, tail_finger, rel, sim1, sim2, sim_dt_h, sim_dt_t):
        ...
        d_g1 = self.Fusion(h1, h_data)
        d_g2 = self.Fusion(h2, t_data)
        pair = torch.cat([d_g1, d_g2], dim=-1)
        pair = pair.float()
        rfeat = self.rmodule(rel)
        logit = (self.lin(pair) * rfeat).sum(-1)
...
class Sim_Finger(nn.Module):
    ...
    def Fusion(self,finger, sim_ht):

        finger_c = finger
        finger1, sim1 = self.scMoAnnoPretrain(finger_c, sim_ht)
        global_feature = finger1 + sim1
        norm_global = nn.BatchNorm1d(global_feature.shape[1]).to(self.device)
        global_feature = norm_global(global_feature)
        return global_feature
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger,tail_finger,rel, sim1, sim2,sim_dt_h,sim_dt_t):
        ...
        d_g1 = self.Fusion(finger1,sim_12)
        d_g2 = self.Fusion(finger2,sim_dt_ht)
        pair = torch.cat([d_g1, d_g2], dim=-1)
        pair = pair.float()
        rfeat = self.rmodule(rel)
        logit = (self.lin(pair) * rfeat).sum(-1)
...
class GNN_Finger(nn.Module):
    ...
    def Fusion(self, sub, data, finger):
    ...
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger,tail_finger,rel, sim1, sim2,sim_dt_h,sim_dt_t):
        ...
        #=======================================
        d_g1 = self.Fusion(h1,h_data,finger1)
        d_g2 = self.Fusion(h2,t_data,finger2)
        pair = torch.cat([d_g1, d_g2], dim=-1)
        pair = pair.float()
        rfeat = self.rmodule(rel)
        logit = (self.lin(pair) * rfeat).sum(-1)
...
def MCF_DDI_net(net_params):
    return MCF_DDI(net_params)
...
class Finger(nn.Module):
    ...
    def forward(self, h_data, t_data, g1, g2, e1, e2, head_finger,tail_finger,rel, sim1, sim2,sim_dt_h,sim_dt_t):
        ...
        finger1,finger2 = self.FingerprintNet(head_finger,tail_finger)
        pair = torch.cat([finger1, finger2], dim=-1)
        pair = pair.float()
        rfeat = self.rmodule(rel)
        logit = (self.lin(pair) * rfeat).sum(-1)
        return logit
def GNN_net(net_params):
    return GNN_(net_params)

def Sim_net(net_params):
    return Sim_(net_params)

def GNN_Sim_net(net_params):
    return GNN_Sim(net_params)

def Sim_Finger_net(net_params):
    return Sim_Finger(net_params)

def Finger_net(net_params):
    return Finger(net_params)


def GNN_Finger_net(net_params):
    return GNN_Finger(net_params)
...
def gnn_model(MODEL_NAME, net_params):
    models = {
        ...
        'Sim_':Sim_net,
        'GNN_Sim': GNN_Sim_net,
        'Sim_Finger': Sim_Finger_net,
        'Finger_': Finger_net,
        'GNN_Finger': GNN_Finger_net,
    }
...
class FingerprintNet_(nn.Module):
    def __init__(self,n_filters=4, output_dim=256, dropout=0.1):
        super().__init__()
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)
        # self.lin = nn.Sequential(
