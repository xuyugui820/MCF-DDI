...
def generate_drug_data(mol_graph, atom_symbols, smiles_rdkit_list,id,smile):
    ...
    data = CustomData(x=features, edge_index=new_edge_index, line_graph_edge_index=line_graph_edge_index,
                      edge_attr=edge_feats, sim=similarity_matrix,drug_target_sim=drug_target_similar_tensor, id=id,smile = smile)
...
def split_data(args):
    ...
    part = save_to_filename.split('/')
    save_to_filename = '/'.join(part[:2])
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(df), 1):
        ...
        train_data.to_csv(f'{save_to_filename}/train_fold_{fold}.csv', index=False)
        val_data.to_csv(f'{save_to_filename}/val_fold_{fold}.csv', index=False)


def save_data_hot(root, fold):
    ...
    train_df = pd.read_csv(os.path.join(root, f'train_fold_{fold}.csv'))
    val_df = pd.read_csv(os.path.join(root, f'val_fold_{fold}.csv'))
    ...
    with open(os.path.join(root, f'train_set_fold_{fold}.pkl'), 'wb') as f:
        pickle.dump(train_set, f)

    with open(os.path.join(root, f'val_set_fold_{fold}.pkl'), 'wb') as f:
...
if __name__ == '__main__':
    ...
    for i in range(5):
        i+=1
        filename = f'{args.dirname}/{args.dataset}'
        save_data_hot(filename,i)
